...
function Connect-NSAppliance {
    ...
    param (
        ...
        [Parameter(Mandatory=$false)] [string]$NSUserName="username", 
        [Parameter(Mandatory=$false)] [string]$NSPassword="userpassword",
...
#     $ManagerAddress = $Receiver.mail
     ...
     #Email structure 
     $msg.From = "no-reply-Netscaler@domain"
...
function Get-NS-Status {
    ...
    $response = Invoke-NSNitroRestApi -NSSession $myNSSession -OperationMethod GET -ResourceType hanode 
    $nsAplianceStatus = $response.hanode[0].$Option
    return $nsAplianceStatus
...
$netscalers = "NSIPaddress01","NSIPaddress02"

foreach ($netscaler in $netscalers)
{
    if (Test-Connection -ComputerName $netscaler -Quiet)
    {   $myNSSession = Connect-NSAppliance -NSAddress $netscaler -NSUserName "username" -NSPassword "userpassword"
        $isPrimary = Get-NS-Status -NSSession $myNSSession -Option state
        if ( $isPrimary -eq "Primary" )
        { break}
        Else { Disconnect-NSAppliance -NSSession $myNSSession }
    }
}
...
$PublicNameToPublish="example.domain.com"
$ServerToPublish="internal_server_fqdn" # FQDN form internal server name
$ServerToPublishIP="internal_server_IP_Address" # internal server IP Address
$ServiceType ="SSL"
